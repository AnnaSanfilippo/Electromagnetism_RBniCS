...
# We also know the exact solution, i.e. u_e=((y^2-1)(z^2-1),(x^2-1)(z^2-1),(x^2-1)(y^2-1)), so we can compute f and as a consequence the absolute and relative Hcurl and the L2 errors.
...
u_e = as_vector(((y**2-1)*(z**2-1),(x**2-1)*(z**2-1),(x**2-1)*(y**2-1)))

# Define variational problem
u = TrialFunction(V)
v = TestFunction(V)
f = as_vector((-2*(y**2 + z**2 - 2) + (y**2 - 1)*(z**2 - 1),
               -2*(x**2 + z**2 - 2) + (x**2 - 1)*(z**2 - 1),
               -2*(x**2 + y**2 - 2) + (x**2 - 1)*(y**2 - 1)))
...
# Save the solution
sol = XDMFFile("Solution_2.xdmf")
u.rename("Sol_N1curl","Sol_N1curl")
sol.write(u)

# Save the exact solution
Pu_e= project(u_e, V)
sol_e = XDMFFile("Exact_Solution_2.xdmf")
