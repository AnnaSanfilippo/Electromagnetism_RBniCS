...
# This is the version built to save the solution in .xml extension. This extension will be used to compute the L2 errors.
...
for meshSize in meshes:
    for k0val in k0s:
        ...
        if mpiRank==0:
            print("#####################################")
            print("Solving: lcar=%.4f   -  k0=%.3f " % (meshSize,k0val))
        ...
        with XDMFFile("data/lcar=%.4f/thermal_block_boundaries.xdmf" % meshSize) as infile:
        ...
        if mpiRank==0:
            ...
            if not os.path.exists(path):
                os.makedirs(path)
        sol2 = File(path+"/Solution_Res.xml")
        sol2 << u
        if mpiRank==0:
            print("########## DONE  #################")
