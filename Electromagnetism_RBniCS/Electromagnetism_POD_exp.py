...
# Electromagnetic problem implemented with the POD version of Reduced Basis and with exponential sampling
...
class Electromagnetism_Solution(EllipticCoerciveCompliantProblem):
    ...
    def compute_theta(self, term):
        mu = self.mu
        if term == "a":
            theta_a0 = 10**(-mu[0]) # For exponential sampling
            theta_a1 = 1.
            theta_a2 = 1.
...
# 2. Create Finite Element space (Edge N1curl)
V = FunctionSpace(mesh, "N1curl", 1)

# 3. Allocate an object of the Electromagnetism class
